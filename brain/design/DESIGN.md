...
- Set: close (X), loop (Preview), check (Saved), info (Save video notes), download (Save video) `M12 4v11M7 10l5 5 5-5M5 20h14` (same 4–20 footprint as the range glyph), stop (Stop saving) `M8 6h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2z` (a 12px rounded square, same 6–18 footprint as close, so it can't be read as the panel's close).
...
### Save video
The panel's **primary** action (Subscribe style). One button that changes in place (like Preview), so focus never has to move. Status (progress or error) goes **above** the button and notes go below it, so the button itself never moves: the panel is anchored to the bottom and grows upwards, so anything added below the button would push it up under the pointer. The idle, saving and converting notes must take the same height for this to hold: `.save > .note { min-height: 54px }` reserves 3 lines. The idle note is exactly 3 lines (1 + 2, see Idle note below); the saving and converting notes may not wrap past 3 at the narrowest panel (keep them short in CONTENT.md).

| State | Above the button (group starts 12px below Preview) | Button | Below the button |
|---|---|---|---|
| Idle | nothing | **Primary**: `--clip-primary-bg`, `--clip-primary-text`, hover `--clip-primary-bg-hover`. Download icon + "Save video" | 8px: idle note (save time with the clip's length, then quality) |
| Unavailable (range invalid) | nothing | Shared disabled style, `aria-disabled`, still focusable, no hover change | 8px: idle note (save time without a length, then quality) |
| Saving | Progress line "Saving 0:03 / 0:07", 8px, progress bar, 8px | **Tonal**: stop icon + "Stop saving", hover `--clip-tonal-hover` | 8px: saving note (replaces the idle note) |
...
| Saved (2s) | nothing | Primary, check icon + "Saved", then back to Idle | 8px: idle note (the browser shows its own download) |
| Failed | Error line (field error style, `--clip-error`, linked by `aria-describedby`), 8px | Back to Idle (primary) immediately, so retry is one press | 8px: idle note |
...
- Idle note (Idle, Unavailable, Saved, Failed): one note block, one 16px info icon, two text parts stacked in one column beside it. 12px / 18px text, icon and text `--clip-text-secondary`, 8px gap to the icon. The whole block is linked to the button with `aria-describedby`, so both parts are read before pressing. It's a hint, not a warning: no colour, no weight change, never an error.
  - **Save-time line** (first, 1 line): Save video plays the clip in real time while it records, so saving takes as long as the clip. With a valid range it states the clip's length (the same `formatTime(end − start)` as the header readout, tabular figures); without one it uses the length-free variant. Wording in CONTENT.md; both variants must fit on one line at the Shorts panel (354px, about 298px of text beside the icon), including an `h:mm:ss` length, so keep them to about 45 characters. It updates silently as the range changes (no announcement; the start announcement already says how long it takes), and because both variants are one line the note's height never changes, so nothing above it moves while the user types a time. In players so narrow that the panel drops below about 300px, a wrap is accepted, as for the other notes.
  - **Quality text** (then, 2 lines): the recording follows the quality selected in YouTube's player, and on "Auto" the resolution can change mid-save. If the main session can tell that the player is on Auto without YouTube internals, show this part only then (the reserved 54px keeps the height; the empty space sits below the save-time line). Otherwise always show it.
  - Why here and not elsewhere: it's where the eye lands just before pressing Save, and the button's description carries it to screen readers. The button label stays "Save video" (a time in the label would read as the clip, not the wait), and nothing is added above the button, where a line appearing when the range turns valid would push the fields up under the user's typing.
- Saving and converting notes: replace the whole idle note (the progress line now shows the time), same style, single text part.
