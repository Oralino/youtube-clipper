...
*Revised 2026-09-24 (file name field):* owner decision. Added the optional File name field as the last field in the fields group, below End and above Preview. It is full width, reuses the time-input tokens and states, has no hint line and no error state, and its placeholder shows the automatic name, truncated with an ellipsis. `maxLength` is 80. Updated the internal layout, the focus order and the input-value type role.
